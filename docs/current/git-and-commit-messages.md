...
One boundary is deliberately not defended: a repository's own native git hooks (`.git/hooks`). Git runs them against the temporary index of a path-limited commit, so a repo-owned `pre-commit` hook that stages files could carry unselected content into sexton's commit. The fleet's synced repos carry no native hooks, and no machinery was added for a case that doesn't exist here — sexton's lifecycle hooks are the supported mechanism. If a hooked repo ever joins the fleet, the known closures are a process-scoped `git -c core.hooksPath=` on regional commits or a temp-index plumbing commit.

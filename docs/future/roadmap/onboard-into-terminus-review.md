...
Most of the "Project Rules" section in `AGENTS.md` is already canonized general-tier material — `no-emoji`, `mermaid-diagrams`, `camelcase-file-naming`, `lowercase-output` all exist as qualities. Today those rules hold only if an agent reads a long orientation file and remembers them; under a rubric they are reviewed. The project-local harvest is the other half of the value: sexton's real invariants (the `GIT_SSH_COMMAND` injection that deliberately never touches a synced repo's git config, the never-silently-discard-the-working-tree guarantee) are the kind of rule whose violation is a bug, and nothing checks them.
